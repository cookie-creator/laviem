...
<div class="row pb-2 px-3 mb-5">
    ...
    @if ($notification->status == 0)
        ...
        <h5 class="pb-1 px-2 text-sm text-black-50">You can edit notification before it sent</h5>

        <div class="col-6 col-md-6">
            <form  method="POST" action="{{ route('notification.update', ['notification_id' => $notification->id]) }}" class="needs-validation" novalidate="">
            @csrf
        ...
        <div class="col-6 col-md-6">
            <form  method="POST" action="{{ route('notification.send', ['notification_id' => $notification->id]) }}" class="needs-validation" novalidate="">
                @csrf
                <div class="col-6">
                    <button class="btn w-50 btn-primary btn-sm" type="submit">Send</button>
                </div>
            </form>
        </div>
    @else
